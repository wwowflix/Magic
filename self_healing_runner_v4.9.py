...
def attempt_self_heal(script_path, error_msg, log_file):
    ...
    try:
        # 1ï¸�â�'£ Handle missing file errors
        ...
        # 3ï¸�â�'£ Handle ImportError / ModuleNotFoundError
...
def main():
    ...
    for script_path in scripts:
        ...
        for attempt in range(1, 4):
            print(f"â�"¶ Running {os.path.basename(script_path)} (attempt {attempt}) ...")
            code, out, err = run_script(script_path, log_file)
            if code == 0:
                print(
                    f"â�"… {os.path.basename(script_path)} completed successfully on attempt {attempt}."
                )
                success = True
                break
            else:
        ...
        if not success:
            print(f"â��' {os.path.basename(script_path)} failed after 3 attempts.")

    with open(summary_path, "w", encoding="utf-8") as summary:
        summary.write("\n".join(summary_lines))

    print(f"\nâ�"… Completed {len(scripts)} scripts. Summary saved to {summary_path}")
