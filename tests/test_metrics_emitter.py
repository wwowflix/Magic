...
def test_metrics_emitter(tmp_path):
    ...
    (out_dir / "phase11_module_C_summary.tsv").write_text(
        "Script\tStatus\nx.py\tPASS\ny.py\tFAIL\n", encoding="utf-8"
    )
    ...
    tools_dir.mkdir()
    (tools_dir / "emit_metrics_from_summaries.py").write_text(
        open(src, "r", encoding="utf-8").read(), encoding="utf-8"
    )

    # Run
    result = subprocess.run(
        [sys.executable, str(tools_dir / "emit_metrics_from_summaries.py")],
        cwd=tmp_path,
        capture_output=True,
        text=True,
    )
