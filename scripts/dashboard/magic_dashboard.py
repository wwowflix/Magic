...
DB = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "outputs", "mydata.db"))

st.set_page_config(page_title="MAGIC Ã¢â‚¬â€œ Trends", layout="wide")
st.title("Ã°Å¸Â§Â MAGIC Ã¢â‚¬â€œ Trends Mini Dashboard")
...
def _detect_ts_col(con):
    ...
    for cand in ["ts", "timestamp", "datetime", "date", "time", "created_at", "fetched_at"]:
        if cand in names:
            return cand
    return None
...
with sqlite3.connect(DB) as con:
    ...
    if not ts_col:
        st.error("`google_trends` has no timestamp-like column. Expected one of: ts, timestamp, datetime, date, time, created_at.\n"
    ...
    if value_col is None:
        st.error("`google_trends` table missing a numeric value column (tried: value, interest, score). "
...
sel_kw = st.sidebar.multiselect("Keywords", kw_options, kw_options[:3] if len(kw_options) >= 3 else kw_options)
sel_rg = st.sidebar.multiselect("Regions", rg_options, rg_options[:2] if len(rg_options) >= 2 else rg_options)
if sel_kw: df = df[df["keyword"].astype(str).isin(sel_kw)]
if sel_rg: df = df[df["region"].astype(str).isin(sel_rg)]

st.caption("Last refresh: **{}Z**".format(datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")))

# Series + chart
df["series"] = df["keyword"].fillna("(none)").astype(str) + " | " + df["region"].fillna("(none)").astype(str)
st.line_chart(df.set_index("ts_dt")[["val_any"]].groupby([df["series"]]).sum())

# Tail table
st.dataframe(df[["ts_dt", "keyword", "region", "val_any"]].tail(25), use_container_width=True)
