...
st.set_page_config(page_title="MAGIC - Ops", layout="wide", page_icon="🪄")
st.title("🪄 MAGIC - Autonomous Ops Dashboard")
...
st.subheader("⚙️ System Source of Truth")

try:
    ms = read_sql("SELECT component, metric, status, details, observed_at FROM magic_status ORDER BY component, metric")
    if ms.empty:
...
st.subheader("📈 Trends (google_trends)")
...
try:
    ...
    if df.empty:
        ...
        pivot = df.pivot_table(index="ts_dt", columns="series", values="value", aggfunc="sum").sort_index()
        st.line_chart(pivot)
