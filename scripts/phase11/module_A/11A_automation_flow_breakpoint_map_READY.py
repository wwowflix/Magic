...
def main():
    try:
        with open(LOG_FILE, "w", encoding="utf-8") as log:
            modules = sorted([d for d in os.listdir(PHASE_PATH) if os.path.isdir(os.path.join(PHASE_PATH, d))])
            for module in modules:
                files = [f for f in os.listdir(os.path.join(PHASE_PATH, module)) if f.endswith("_READY.py")]
                if len(files) == 0:
