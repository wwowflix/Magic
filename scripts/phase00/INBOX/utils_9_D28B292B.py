...
def get_platform_architecture(chrome_version=None):
    if sys.platform.startswith("linux") and sys.maxsize > 2**32:
        ...
        if pf.processor() == "arm":
            if (
                chrome_version is not None
                and get_major_version(chrome_version) >= "115"
            ):
                print("CHROME >= 115, using mac-arm64 as architecture identifier")
                architecture = "-arm64"
            elif chrome_version is not None and version.parse(
                chrome_version
            ) <= version.parse("106.0.5249.21"):
                print(
                    "CHROME <= 106.0.5249.21, using mac64_m1 as architecture identifier"
                )
                architecture = "64_m1"
            else:
                architecture = "_arm64"
...
def get_matched_chromedriver_version(chrome_version, no_ssl=False):
    ...
    if get_major_version(chrome_version) >= "115":
        ...
        milestone = latest_version_per_milestone["milestones"].get(
