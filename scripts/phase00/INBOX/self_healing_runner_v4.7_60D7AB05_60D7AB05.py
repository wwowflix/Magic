...
def apply_remediation(error_msg, script_path):
    ...
    if "FileNotFoundError" in error_msg:
        ...
        print(
            "ðŸ”§ Auto-fix applied: UnicodeDecodeError detected (manual cleanup recommended)"
        )
        return False
        ...
        if mod_match:
            ...
            print(
                f"ðŸ”§ Auto-fix applied: installing missing module '{module}' via pip"
            )
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if apply_remediation(result.stderr, script_path):
                        print(
                            f"ðŸ”§ Remediation applied, retrying script: {script_name}"
                        )
                        continue
                    else:
                        if attempt == MAX_RETRIES:
                            print(
                                f"âŒ {script_name} failed after {MAX_RETRIES} attempts."
                            )
                            return 1
                        else:
                            continue
                ...
                if attempt == MAX_RETRIES:
                    print(
                        f"âŒ {script_name} failed after {MAX_RETRIES} attempts due to exception."
                    )
                    return 1
