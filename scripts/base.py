    ...
    if isinstance(obj, _JSONDataclass) or dataclasses.is_dataclass(obj):
        ...
        for field in dataclasses.fields(obj):
            ...
            out[field.name] = _json_dataclass_to_dict(
                getattr(obj, field.name), forBuggyIntParser=forBuggyIntParser
            )
        # Add properties
        for k in dir(obj):
            if isinstance(getattr(type(obj), k, None), (property, _DeprecatedProperty)):
                ...
                out[k] = _json_dataclass_to_dict(
                    getattr(obj, k), forBuggyIntParser=forBuggyIntParser
                )
    elif isinstance(obj, (tuple, list)):
        return type(obj)(
            _json_dataclass_to_dict(x, forBuggyIntParser=forBuggyIntParser) for x in obj
        )
