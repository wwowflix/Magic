...
def read_trends(conn: sqlite3.Connection, since_utc: datetime | None = None) -> pd.DataFrame:
...
def force_refresh_trends(default_keywords: list[str] | None = None, regions: list[str] | None = None, timeframe: str = "now 7-d") -> subprocess.CompletedProcess | None:
    ...
    if not kw:
        try:
            with sqlite3.connect(DB_PATH) as c:
                dfk = pd.read_sql_query("SELECT DISTINCT keyword FROM google_trends ORDER BY keyword", c)
    ...
    return subprocess.run(args, capture_output=True, text=True, encoding="utf-8", cwd=ROOT)
...
refresh_interval_sec = st.sidebar.number_input("Auto-refresh (seconds)", min_value=10, max_value=600, value=60, step=10)
if UNDER_PYTEST:
    refresh_interval_sec = 0
st_autorefresh = getattr(st, 'rerun', None) or getattr(st, 'experimental_rerun', None)  # placeholder; below we use st.experimental_data_editor trick
...
try:
    ...
    if os.environ.get('PYTEST_CURRENT_TEST'):
        ...
        conn.execute('CREATE TABLE IF NOT EXISTS google_trends (keyword TEXT, region TEXT, ts TEXT, value REAL)')
    else:
        raise
...
with st.sidebar:
    ...
    try:
        df_meta = pd.read_sql_query("SELECT DISTINCT keyword, region FROM google_trends", conn)
    ...
    sel_keywords = st.multiselect("Keywords", options=all_keywords, default=all_keywords[:3] if all_keywords else [])
    sel_regions = st.multiselect("Regions", options=all_regions, default=all_regions[:2] if all_regions else [])
    ...
    with colA:
        if st.button("🔄 Force Trends Refresh"):
            res = force_refresh_trends(sel_keywords or None, sel_regions or None, timeframe=f"now {days}-d")
            if res is not None:
                if res.returncode == 0:
                    st.success("Collector ran successfully.")
                else:
                    st.error(f"Collector failed (code {res.returncode}). See logs below.")
...
with col2:
    ...
    last_ts_local = last_ts.astimezone(LOCAL_TZ).strftime("%Y-%m-%d %H:%M:%S") if last_ts else "-"
...
if df.empty:
    st.info("No data for the selected window/filters. Try widening the time range or removing filters.")
else:
    ...
    fig = px.line(df_line, x="ts_local", y="value", color="Series", hover_data=["keyword", "region"], markers=True)
    fig.update_layout(xaxis_title="Time (local)", yaxis_title="Value", legend_title="Series")
    st.plotly_chart(fig, use_container_width=True)

    # Acceleration metric: last N vs previous N window mean
    st.subheader("🚀 Top Trending by Acceleration")
    accel_window = st.number_input("Acceleration window (hours)", min_value=1, max_value=24, value=6, step=1)
    ...
    def compute_accel_safe(frame: pd.DataFrame, hours: int) -> pd.DataFrame:
        ...
        for (k, r), g in frame.groupby(["keyword", "region" ], dropna=False):
            ...
            early = g[(g["ts_dt"] >= early_start) & (g["ts_dt"] < late_start)]["value"].mean()
            late = g[g["ts_dt"] >= late_start]["value"].mean()
            if pd.notna(early) and pd.notna(late):
                delta = late - early
                pct = (delta / early * 100.0) if early else np.nan
                out.append({"keyword": k, "region": r, "late_mean": late, "early_mean": early, "delta": delta, "%chg": pct})
        out_df = pd.DataFrame(out, columns=["keyword","region","late_mean","early_mean","delta","%chg"])
        if out_df.empty:
            return out_df
    ...
    if accel_df.empty:
        st.info("Not enough data density to compute acceleration. Let the collector run a bit more.")
    else:
...
# Friendly footer
st.caption("MAGIC Dashboard v1 â€¢ Live filters, acceleration insights, scheduler status, and export. ✨")
