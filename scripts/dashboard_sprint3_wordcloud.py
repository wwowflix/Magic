...
st.title("🌟 Zephyr Trend Dashboard - Keyword Word Cloud")
...
query = f"""
SELECT keyword, SUM(metric) as total_metric FROM trends
...
# Generate word cloud
text_freq = dict(zip(df['keyword'], df['total_metric']))
wordcloud = WordCloud(width=800, height=400, background_color="white").generate_from_frequencies(text_freq)
