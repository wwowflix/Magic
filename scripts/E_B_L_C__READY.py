...
class table_E_B_L_C_(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        ...
        sstruct.unpack(eblcHeaderFormat, data[:8], self)
        i += 8
        ...
        for curStrikeIndex in range(self.numSizes):
            ...
            sstruct.unpack2(bitmapSizeTableFormatPart1, data[i : i + 16], curTable)
            i += 16
            for metric in ("hori", "vert"):
                metricObj = SbitLineMetrics()
                vars(curTable)[metric] = metricObj
                sstruct.unpack2(sbitLineMetricsFormat, data[i : i + 12], metricObj)
                i += 12
            sstruct.unpack(bitmapSizeTableFormatPart2, data[i : i + 8], curTable)
            i += 8
