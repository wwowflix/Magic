...
from PIL.Image import Image, Resampling, Transform
...
class ImageCaptcha:
    ...
    def create_noise_dots(
        image: Image, color: ColorTuple, width: int = 3, number: int = 30
    ) -> Image:
        draw = Draw(image)
        w, h = image.size
        while number:
    ...
    def _draw_character(self, c: str, draw: ImageDraw, color: ColorTuple) -> Image:
        ...
        dx2 = (
            w
            * (secrets.randbits(32) / (2**32))
            * (self.character_warp_dx[1] - self.character_warp_dx[0])
            + self.character_warp_dx[0]
        )
        dy2 = (
            h
            * (secrets.randbits(32) / (2**32))
            * (self.character_warp_dy[1] - self.character_warp_dy[0])
