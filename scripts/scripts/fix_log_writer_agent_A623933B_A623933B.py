...
# ------------------------------
# 1ï¸ƒ£ Ensure logs directory exists
# ------------------------------
os.makedirs(LOGS_DIR, exist_ok=True)

# ------------------------------
# 2ï¸ƒ£ Ensure target script exists
# ------------------------------
if not os.path.exists(TARGET_SCRIPT):
    ...
    print(f"œ… Script found: {TARGET_SCRIPT}")

# ------------------------------
# 3ï¸ƒ£ Write to log
# ------------------------------
timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
log_entry = f"[{timestamp}] œ… Checked & ensured script exists: {TARGET_SCRIPT}\n"

with open(MASTER_LOG, "a", encoding="utf-8") as log:
    log.write(log_entry)

print(f"ðŸ" Log updated: {MASTER_LOG}")
print("œ… Fix Log Writer Agent process completed.")
