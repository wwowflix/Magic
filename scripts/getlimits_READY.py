...
class MachArLike:
    ...
    def __init__(self, ftype, *, eps, epsneg, huge, tiny, ibeta, **kwargs):
        ...
        def float_conv(v):
            return array([v], ftype)

        def float_to_float(v):
            return _fr1(float_conv(v))

        def float_to_str(v):
            return params["fmt"] % array(_fr0(v)[0], ftype)
