...
def classify_text(text: str) -> str:
    ...
    if "raise notimplementederror" in blob:
        return "placeholder"
    only_defs = all(
        ln.startswith(("def ", "class ", "#")) or ln == "pass" for ln in nonblank
    )
...
def scan():
    ...
    summary = {
        ...
        "coverage_overall_pct": (
            round((totals["covered_lines"] / totals["total_lines"] * 100.0), 1)
    ...
    print(
        f"Real: {totals.get('real',0)} | Placeholder: {totals.get('placeholder',0)} | Empty: {totals.get('empty',0)} | Broken: {totals.get('broken',0)}"  # noqa: E501
    )
    print(
        f"Coverage: {summary['coverage_overall_pct']}% (lines: {totals['covered_lines']}/{totals['total_lines']})"  # noqa: E501
    )
    print(f"Wrote: {tsv}")
