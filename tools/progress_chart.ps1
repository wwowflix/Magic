...
if (-not (Test-Path $json) -and -not (Test-Path $tsv)) { throw "No progress_status.{json|tsv} found in $reports. Run verify_release_progress.ps1 first." }
if (Test-Path $json) {
...
$pass=[int]$summary.pass; $total=[int]$summary.total; $fail=$total-$pass; $pct=[double]$summary.percent
Write-Host ""
Write-Host ("MAGIC Progress Chart @ {0} — {1}/{2} ({3}%)" -f $Root,$pass,$total,$pct) -ForegroundColor Cyan
$barLen=50; $passLen=[int][math]::Round(($pass/[double][math]::Max($total,1))*$barLen); $failLen=$barLen-$passLen
$bar=('█'*$passLen)+('░'*$failLen); Write-Host ("[{0}] PASS={1}  FAIL={2}" -f $bar,$pass,$fail)
$failRows = $results | Where-Object { $_.Status -eq 'FAIL' } | Sort-Object {[int]$_.Id}
if ($failRows) {
  Write-Host ""; Write-Host "Top failing steps:" -ForegroundColor Yellow
...
$bg=[System.Drawing.Brushes]::White; $gfx.FillRectangle($bg,0,0,$width,$height)
...
$y=170; foreach($row in $failRows | Select-Object -First 6){ $txt = ("#{0}  {1}" -f $row.Id,$row.Step); $gfx.DrawString($txt,$fontBody,$red,24,$y); $y+=20 }
$bmp.Save($png,[System.Drawing.Imaging.ImageFormat]::Png)
$gfx.Dispose(); $bmp.Dispose()
Write-Host ""; Write-Host "Chart saved: $png" -ForegroundColor DarkGray
