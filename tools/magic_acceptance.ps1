...
$pyNote = if ($pyExists) { $PyExe } else { "" }
Show "venv Python found"  $pyExists $pyNote
...
# 5) Phase 0 sorter
$sorterExists = Test-Path $Sorter
Show "0A_sorter_READY.py present" $sorterExists
if ($sorterExists -and $pyExists) {
  try {
