import sys, os, re, importlib, pathlib, json, time
...
RISKY_BASENAMES = {
    ...
    "_asarray", "multiarray", "overrides", "_add_newdocs", "_array_function",
    "fromnumeric", "_methods", "numeric", "_dtype", "_ufunc_config",
}
PRIVATE_PATTERNS = [
...
def scan_repo():
    ...
    for p in SCRIPTS.glob("**/*.py"):
        ...
        if base in RISKY_BASENAMES:
            rows.append(("SHADOW_NAME", str(p.relative_to(ROOT)), "High",
        ...
        for pat in PRIVATE_PATTERNS:
            if re.search(pat, text):
                rows.append(("PRIVATE_NUMPY_API", str(p.relative_to(ROOT)), "High",
                             f"Matches pattern: {pat}"))

        for pat in RELATIVE_PRIVATE:
            if re.search(pat, text):
                rows.append(("RELATIVE_PRIVATE_IMPORT", str(p.relative_to(ROOT)), "High",
    ...
    def origin(name):
        try:
            ...
            return getattr(m, "__file__", "<built-in/extension>") or "<built-in/extension>"
        except Exception as e:
    ...
    runtime = [
        ...
        ("RUNTIME_IMPORT", "numpy.core._multiarray_umath", origin("numpy.core._multiarray_umath")),
        ("RUNTIME_IMPORT", "numpy._core.function_base", origin("numpy._core.function_base")),
        ("RUNTIME_IMPORT", "numpy.core.function_base", origin("numpy.core.function_base")),
    ]
...
def write_tsv(env, rows, runtime):
    with open(tsv, "w", encoding="utf-8", newline="") as f:
        ...
        f.write(f"ENV\tnumpy_version\tinfo\t{env.get('numpy_version', env.get('numpy_error','?'))}\n")
        f.write(f"ENV\tnumpy_file\tinfo\t{env.get('numpy_file','?')}\n")
        f.write(f"ENV\tPYTHONPATH\tinfo\t{env.get('PYTHONPATH','')}\n")
        f.write(f"ENV\tsys.path[0:5]\tinfo\t{json.dumps(env.get('sys_path_head', []))}\n")
