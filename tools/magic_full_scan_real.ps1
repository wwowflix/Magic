...
Write-Host "MAGIC full scanner -- LIVE build v1.4" -ForegroundColor Yellow
...
New-Dir $reportsDir
New-Dir $readinessDir

# Python optional (older PS syntax)
$pyCmd = Get-Command python -ErrorAction SilentlyContinue
if ($pyCmd) { $PYEXE = $pyCmd.Source } else { $PYEXE = $null }

# Enumerate ALL .py in scripts (exclude tests)
$files = Get-ChildItem -Path (Join-Path $scriptsDir '*') -Recurse -File -Include *.py -ErrorAction Stop |
  Where-Object {
    $_.FullName -notmatch '(?i)[/\\]tests[/\\]' -and
    $_.Name     -notmatch '(^test_|_test\.py$)'
  }

# Narrow to MAGIC patterns and exclude common noise
...
foreach ($f in $files) {
  ...
  $m1 = [regex]::Match($rel, "(?i)phase[\\/_-]?(\d{1,2})")
  if ($m1.Success) { $phase = [int]$m1.Groups[1].Value }
  if (-not $phase) {
    $m2 = [regex]::Match($f.Name, "^(\d{1,2})[A-Z]_")
    if ($m2.Success) { $phase = [int]$m2.Groups[1].Value }
  }
  $m3 = [regex]::Match($rel, "(?i)module[\\/_-]?([a-z])")
  if ($m3.Success) { $module = $m3.Groups[1].Value.ToUpper() }
  if (-not $module) {
    $m4 = [regex]::Match($f.Name, "^\d{1,2}([A-Z])_")
    if ($m4.Success) { $module = $m4.Groups[1].Value.ToUpper() }
  }
  ...
  # Syntax check (safe; do not terminate on stderr)
  $syntaxOK  = $true
  $syntaxMsg = ""
  if ($PYEXE) {
    $escaped = $f.FullName.Replace("'", "''")
    $cmd = "import py_compile,sys; p=r'${escaped}'; " +
           "try:\n py_compile.compile(p, doraise=True); print('OK'); sys.exit(0)\n" +
    ...
    if ($exit -ne 0) { $syntaxOK = $false; $syntaxMsg = ($out | Out-String).Trim() }
  }

  $status = "PASS"; $notes = "implemented"
  if (-not $syntaxOK) { $status = "WARN"; $notes = "syntax check failed (temp-warn)" }
  elseif ($placeholder.Count -gt 0) { $status = "WARN"; $notes = "placeholder: " + ($placeholder -join "; ") }
if(-not $phase){ $phase = 0 }
...
# Build report safely even when empty
...
[System.IO.File]::WriteAllText($jsonPath,   $js, $enc)
[System.IO.File]::WriteAllText($jsonPathTS, $js, $enc)
...
$tsv = ($lines -join "`r`n")
[System.IO.File]::WriteAllText($tsvPath,   $tsv, $enc)
[System.IO.File]::WriteAllText($tsvPathTS, $tsv, $enc)

# Readiness (robust when empty)
...
foreach ($pp in ($perPhase | Sort-Object {[int]$_.phase})) {
  ...
  $note = ("pass={0}; warn={1}; error={2}; total={3}; progress={4}%" -f $pp.pass,$pp.warn,$pp.error,$pp.total,$pp.progress)
...
[System.IO.File]::WriteAllText($readyNow, $readyText, $enc)
[System.IO.File]::WriteAllText($readyTS,  $readyText, $enc)
...
[System.IO.File]::WriteAllText($quickJSON, ($quick | ConvertTo-Json -Depth 6), $enc)
...
[System.IO.File]::WriteAllLines($quickTSV, $quickLines, $enc)

if (-not $Quiet) {
  Write-Host ("Found {0} Python scripts; PASS:{1} WARN:{2} ERROR:{3}" -f $report.total_scripts,$report.pass,$report.warn,$report.error) -ForegroundColor Green
...
exit 0.FullName -notmatch '(?i)[/\\]\.git[/\\]|[/\\](?:venv|\.venv|env|site-packages|outputs|build|dist|node_modules)[/\\]'
  -and
  ([CmdletBinding()]
param(
  [string]$Root = (Get-Location).Path,
...
exit 0.Name -match '^(?:\d{1,2}[A-Z]_.*|.*_READY\.py|Blocks\.py|ElementTree\.py|soupparser\.py|G_D_E_F_\.py|CFF2ToCFF\.py|CFFToCFF2\.py)
