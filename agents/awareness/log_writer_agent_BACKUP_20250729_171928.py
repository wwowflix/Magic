...
def run_and_log(script_path):
    ...
    if not os.path.exists(abs_path):
        write_log(f"â��' Target script not found: {abs_path}")
        return

    write_log(f"ðŸš€ Starting script: {abs_path}")
    try:
        result = subprocess.run(["python", abs_path], capture_output=True, text=True, check=False)
        if result.stdout:
            write_log(f"â�"… OUTPUT:\n{result.stdout.strip()}")
        if result.stderr:
            write_log(f"âš ï¸� ERRORS:\n{result.stderr.strip()}")
    except Exception as e:
        write_log(f"â��' Failed to run script {abs_path}: {str(e)}")
    finally:
        write_log(f"ðŸ�� Finished script: {abs_path}\n")
