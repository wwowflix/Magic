    ...
    if isinstance(path_value, str):
        ...
        if not os.path.exists(expected_path):
            missing_files.append(
                f'{row["Phase"]} | {row["Module"]} | {row["Filename"]} â†’ MISSING'
            )
...
with open(LOG_OUTPUT, "w", encoding="utf-8") as f:
    if not missing_files:
        f.write("âœ… All expected scripts are present.\n")
    else:
        f.write("âŒ Missing Scripts Detected:\n")
        for entry in missing_files:
            f.write(f"{entry}\n")

print(f"ðŸ” Scan complete. Missing entries: {len(missing_files)}")
